...
- **All source status transitions go through `Source::StatusMachine`.** Never set `source.status` directly outside this service or the `Source.follow!` creation path. `:pending` → `:ok`/`:degraded`/`:recovering`/`:failed`; transient errors go to `:recovering` (backoff retried by the sweep), hard errors to `:failed` (manual pull).
