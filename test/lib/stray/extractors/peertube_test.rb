...
class Stray::Extractors::PeertubeTest < ActiveSupport::TestCase
  ...
  def stub_json(body)
    ...
    extractor = Stray::Extractors::Peertube.new
    extractor.define_singleton_method(:fetch) { |_url| resp }
    yield extractor
  end

  test "matches? returns true for video-channels URLs" do
    assert Stray::Extractors::Peertube.matches?("https://tilvids.com/video-channels/fedi")
    assert Stray::Extractors::Peertube.matches?("https://tilvids.com/c/fedi")
  end

  test "matches? returns false for generic URLs" do
    assert_not Stray::Extractors::Peertube.matches?("https://tilvids.com/videos/trending")
  end

  test "channel_handle parses video-channels and c paths" do
    assert_equal "fedi", Stray::Extractors::Peertube.channel_handle("https://tilvids.com/video-channels/fedi")
    assert_equal "fedi", Stray::Extractors::Peertube.channel_handle("https://tilvids.com/c/fedi")
  end
