...
class Stray::Bridges::RumbleTest < ActiveSupport::TestCase
  FIXTURE = File.expand_path("../../../fixtures/files/rumble_channel.html", __dir__)

  def stub_html(body)
    resp = OpenStruct.new(status: 200, body: body)
    extractor = Stray::Bridges::Rumble.new
    extractor.define_singleton_method(:fetch) { |_url| resp }
    yield extractor
  end

  test "matches? returns true for rumble URLs" do
    assert Stray::Bridges::Rumble.matches?("https://rumble.com/c/BrightInsight")
    assert Stray::Bridges::Rumble.matches?("https://rumble.com/v7a8neu.html")
  end

  test "matches? returns false for non-rumble URLs" do
    assert_not Stray::Bridges::Rumble.matches?("https://example.com")
  end

  test "channel_slug parses /c/ and /user/ URLs" do
    assert_equal "BrightInsight", Stray::Bridges::Rumble.channel_slug("https://rumble.com/c/BrightInsight")
    assert_equal "samtime", Stray::Bridges::Rumble.channel_slug("https://rumble.com/user/samtime")
  end
