...
class Extractors::YtDlpTest < ActiveSupport::TestCase
  ...
  test "matches? returns false for YouTube RSS feed URLs (handled by YoutubeRss)" do
    assert_not Extractors::YtDlp.matches?("https://www.youtube.com/feeds/videos.xml?channel_id=UC123")
  end

  test "handles_kind? returns true for video_channel" do
    assert Extractors::YtDlp.handles_kind?("video_channel")
  end

  test "handles_kind? returns false for other kinds" do
    assert_not Extractors::YtDlp.handles_kind?("youtube_channel")
