...
class Extractors::GenericPageTest < ActiveSupport::TestCase
  test "matches? returns true for any http/https URL" do
    assert Extractors::GenericPage.matches?("https://example.com/blog/post-1")
    assert Extractors::GenericPage.matches?("http://example.com/about")
  end

  test "matches? returns false for non-http URLs" do
    assert_not Extractors::GenericPage.matches?("ftp://example.com/file")
    assert_not Extractors::GenericPage.matches?("not a url")
  end

  test "handles_kind? returns true for generic_page" do
    assert Extractors::GenericPage.handles_kind?("generic_page")
  end

  test "handles_kind? returns false for other kinds" do
    assert_not Extractors::GenericPage.handles_kind?("rss_feed")
